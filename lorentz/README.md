...
Monero is a 100% community-sponsored endeavor. If you want to join our efforts, the easiest thing you can do is support the project financially. Both Monero and Bitcoin donations can be made to **donate.getlorentz.org** if using a client that supports the [OpenAlias](https://openalias.org) standard. Alternatively you can send XMR to the Monero donation address via the `donate` command (type `help` in the command-line wallet for details).
...
To list all available options, run `./bin/lorentzd --help`.  Options can be
...
To run in background:
    ...
    ./bin/lorentzd --log-file lorentzd.log --detach
...
lorentz-wallet-cli, and possibly lorentzd, if you get crashes refreshing.
...
* `--p2p-bind-ip 127.0.0.1` on the command line or `p2p-bind-ip=127.0.0.1` in
  lorentzd.conf to disable listening for connections on external interfaces.
* `--no-igd` on the command line or `no-igd=1` in lorentzd.conf to disable IGD
  (UPnP port forwarding negotiation), which is pointless with Tor.
* `DNS_PUBLIC=tcp` or `DNS_PUBLIC=tcp://x.x.x.x` where x.x.x.x is the IP of the
  desired DNS server, for DNS requests to go over TCP, so that they are routed
  through Tor. When IP is not specified, lorentzd uses the default list of
  servers defined in [src/common/dns_utils.cpp](src/common/dns_utils.cpp).
* `TORSOCKS_ALLOW_INBOUND=1` to tell torsocks to allow lorentzd to bind to interfaces
...
* Do NOT pass `--detach` when running through torsocks with systemd, (see
  [utils/systemd/lorentzd.service](utils/systemd/lorentzd.service) for details).

Example command line to start lorentzd through Tor:

    DNS_PUBLIC=tcp torsocks lorentzd --p2p-bind-ip 127.0.0.1 --no-igd
...
allow inbound connections. Full example:
    ...
    DNS_PUBLIC=tcp torsocks ./lorentzd --p2p-bind-ip 127.0.0.1 --no-igd --rpc-bind-ip 127.0.0.1 \
...
```
gdb /path/to/lorentzd `pidof lorentzd` 
```
...
When it terminates with an output along the lines of "Segmentation fault (core dumped)", there should be a core dump file in the same directory as lorentzd. It may be named just `core`, or `core.xxxx` with numbers appended.

You can now analyse this core dump with `gdb` as follows:

`gdb /path/to/lorentzd /path/to/dumpfile`
...
Type `run` to run lorentzd

### Analysing memory corruption

We use the tool `valgrind` for this.

Run with `valgrind /path/to/lorentzd`. It will be slow.
