...
Core tests take longer than any other Lorentz tests, due to the high amount of computational work involved in validating core components.
...
To run only Lorentz's core tests (after building):
...
To run only Lorentz's crypto tests (after building):
...
To run only Lorentz's hash tests (after building):
...
To run only Lorentz's performance tests (after building):
...
To run only Lorentz's unit tests (after building):
