if(APPLE OR (WIN32 AND NOT STATIC))
    ...
    if(APPLE AND NOT IOS)
        ...
        # Help macdeployqt resolve transitive @rpath dependencies.
        find_package(Boost QUIET COMPONENTS atomic container date_time)
        find_package(Protobuf QUIET)
        set(_macdeployqt_library_options)
        foreach(_tgt Boost::atomic Boost::container Boost::date_time protobuf::libprotobuf)
            if(TARGET ${_tgt})
                list(APPEND _macdeployqt_library_options
                    "-libpath=$<TARGET_FILE_DIR:${_tgt}>"
                )
            endif()
        endforeach()

        # macdeployqt skips Boost's @loader_path dependencies. Copy them first.
        foreach(_tgt Boost::atomic Boost::container Boost::date_time)
            if(TARGET ${_tgt})
        ...
        find_library(_abseil_strings_dylib
            NAMES absl_strings
            HINTS /opt/homebrew/opt/abseil/lib /usr/local/opt/abseil/lib
        )
        if(_abseil_strings_dylib)
            get_filename_component(_abseil_lib_dir "${_abseil_strings_dylib}" DIRECTORY)
            list(APPEND _macdeployqt_library_options "-libpath=${_abseil_lib_dir}")
        endif()

        add_custom_command(TARGET deploy
            POST_BUILD
            COMMAND "${MACDEPLOYQT_EXECUTABLE}" "$<TARGET_FILE_DIR:monero-wallet-gui>/../.."
                -always-overwrite "-qmldir=${CMAKE_SOURCE_DIR}" ${_macdeployqt_library_options}
