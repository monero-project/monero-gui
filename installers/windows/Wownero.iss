...
AppVersion=0.12.0.0
DefaultDirName={pf}\Wownero GUI Wallet
DefaultGroupName=Wownero GUI Wallet
UninstallDisplayIcon={app}\wownero-wallet-gui.exe
...
; As of version 0.12.0.0, the Wownero .exe files do not carry version info anyway in their .exe headers.
...
; Wownero GUI wallet
Source: "bin\wownero-wallet-gui.exe"; DestDir: "{app}"; Flags: ignoreversion

; Wownero GUI wallet log file
...
Source: "wownero-wallet-gui.log"; DestDir: "{app}"; Flags: onlyifdoesntexist; Permissions: users-modify

; Wownero CLI wallet
Source: "bin\wownero-wallet-cli.exe"; DestDir: "{app}"; Flags: ignoreversion

; Wownero wallet RPC interface implementation
Source: "bin\wownero-wallet-rpc.exe"; DestDir: "{app}"; Flags: ignoreversion
...
Source: "bin\wownero-blockchain-export.exe"; DestDir: "{app}"; Flags: ignoreversion
Source: "bin\wownero-blockchain-import.exe"; DestDir: "{app}"; Flags: ignoreversion

; was present in 0.10.3.1, not present anymore in 0.11.1.0 and after
; Source: "bin\wownero-utils-deserialize.exe"; DestDir: "{app}"; Flags: ignoreversion
...
; Filename: "{app}\wownero-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  Result := BlockChainDir('') + '\wownerod.log';
...
begin
  if CurStep = ssPostInstall then begin
    ...
    s[1] := '"' + ExpandConstant('{app}\wownerod.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
...
Name: "{group}\GUI Wallet"; Filename: "{app}\wownero-wallet-gui.exe"
...
Name: "{group}\Utilities\Wownero Daemon"; Filename: "{app}\wownerod.exe"; Parameters: {code:DaemonFlags}
...
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: "{app}\wownero-wallet-gui.log"
...
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM wownerod.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{userdesktop}\GUI Wallet"; Filename: "{app}\wownero-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\wownero-project\wownero-core"; ValueType: string; ValueName: "daemonFlags"; ValueData: {code:DaemonFlags};
