#!/bin/bash
MONERO_URL=https://github.com/lorentz-project/lorentz.git
...
git -C $MONERO_DIR config user.name "Lorentz GUI"
git -C $MONERO_DIR config user.email "gui@lorentz.local"
...
for PR in $(git log --format=%B -n 1 | grep -io "requires #[0-9]*" | sed 's/[^0-9]*//g'); do
    ...
    git -C $MONERO_DIR merge --quiet PR-$PR  -m "Merge lorentz PR #$PR"
...
## Compare previously built version with submodule + merged PR's version. 
    ...
    if [ "$VERSIONTAG" != "$GUI_MONERO_VERSION" ]; then
        ...
        echo "latest libwallet ($GUI_MONERO_VERSION) is already built. Remove lorentz/lib/libwallet_merged.a to force rebuild"
    fi
fi
