...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/wownero-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    WOWNERO_EXEC=wownerod.exe
fi
...
get_tag
popd
echo "var GUI_WOWNERO_VERSION = \"$TAGNAME\"" >> version.js
...
$QMAKE ../wownero-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy wownerod to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$WOWNERO_DIR/bin/$WOWNERO_EXEC $BIN_PATH
fi
